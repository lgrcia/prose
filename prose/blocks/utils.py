...
class Calibration_YS(Block):
        ...
        if images is None:
            ...
            for image in images:
                ...
                if image_type == "bias":
                    ...
                    if self.calibration_method == 'standard':
                        _dark = (image_data - self.master_bias) / image_exposure
                    elif self.calibration_method == 'darkonly':
                        _dark = image_data / image_exposure
                    _master.append(_dark)
                elif image_type == "flat":
                    if self.calibration_method == 'standard':
                        _flat = (
                        ...
                        _flat = image_data - self.master_dark * image_exposure
                    _flat /= np.mean(_flat)
                    _master.append(_flat)
                    del image_data
    ...
    def _calibration_shared(self, image, exp_time):
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ...
            if self.calibration_method == 'darkonly':
                return (image - dark * exp_time) / (flat)

    def _calibration(self, image, exp_time):
        with np.errstate(divide="ignore", invalid="ignore"):
            if self.calibration_method == 'standard':
                return (                                            #implement calibration method
                image - (self.master_dark * exp_time + self.master_bias)
            ) / self.master_flat
            if self.calibration_method == 'darkonly':
                return (image - self.master_dark * exp_time) / self.master_flat
