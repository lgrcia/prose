...
class FitsManager:
    ...
    def print(self, calib=True, repr=False, exposure=False, **kwargs):
        ...
        obs = self.observations(imtype="light", show=False, index=False, order_exposure=exposure, **updated_kwargs)
        ...
        others = self.observations(show=False, index=False, order_exposure=exposure, **updated_kwargs)
        obs_and_calibs = [*obs, *calibs]
    ...
    def update_observations(self):
        observations = self.to_pandas(f"select {UNIQUE_FIELDS} from files WHERE observation is NULL GROUP BY {UNIQUE_FIELDS}").values
        ...
        for obs in observations:
            ...
            obs_id = self.con.execute(f"SELECT id FROM observations where {query}").fetchall()[0]

            # and fill files id values
            self.con.execute(f"UPDATE files SET observation = ? WHERE observation is NULL AND {query}", [obs_id])
            files_updated = self.con.execute("select changes()").fetchall()[0]
