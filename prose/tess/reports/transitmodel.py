...
class TransitModel(TFOPObservation, LatexTemplate):
    ...
    def __init__(self, obs, transit, trend=None, expected=None, posteriors={}, rms_bin=5/24/60, name=None,
    ...
    def plot_lc_model(self):
        ...
        plt.plot(self.obs.time, self.trend_model + self.transit_model, c="C0",
                 label="systematics + transit model")
        plt.plot(self.obs.time, self.transit_model + 1. - 0.03, label="transit model", c="k")
        plt.text(plt.xlim()[1] + 0.002, 1, "RAW", rotation=270, ha="center")
        viz.plot(self.obs.time, self.obs.diff_flux - self.trend_model + 1. - 0.03)
    ...
    def get_priors(self):
        priors = {}
        if self.obs.priors_dataframe is not None:
            priors['rad_p'] = self.obs.priors_dataframe['Planet Radius (R_Earth)'][0]
            priors['rad_p_e'] = self.obs.priors_dataframe['Planet Radius (R_Earth) err'][0]
            priors['rad_s'] = self.obs.priors_dataframe['Stellar Radius (R_Sun)'][0]
            priors['rad_s_e'] = self.obs.priors_dataframe['Stellar Radius (R_Sun) err'][0]
            priors['mass_s'] = self.obs.priors_dataframe['Stellar Mass (M_Sun)'][0]
            priors['mass_s_e'] = self.obs.priors_dataframe['Stellar Mass (M_Sun) err'][0]
            priors['period'] = self.obs.priors_dataframe['Period (days)'][0]
            priors['period_e'] = self.obs.priors_dataframe['Period (days) err'][0]
            priors['duration'] = self.obs.priors_dataframe['Duration (hours)'][0] * 60
            priors['duration_e'] = self.obs.priors_dataframe['Duration (hours) err'][0] * 60
            priors['depth'] = self.obs.priors_dataframe['Depth (ppm)'][0] / 1e3
            priors['depth_e'] = self.obs.priors_dataframe['Depth (ppm) err'][0] / 1e3
        else:
            priors['rad_p'] = np.nan
